...
from second_brain.repo_intel.state import AgentState, RepoIntelMode
...
async def _call_tool(name: str, payload: Any) -> Any:
    ...
    try:
        ...
        if name == "rag_indexer":
            from second_brain.repo_intel.rag import IndexRepositoryInput, index_repository

            return await index_repository(IndexRepositoryInput.model_validate(payload))
        if name == "rag_retriever":
            from second_brain.repo_intel.rag import RetrieveInput, retrieve
